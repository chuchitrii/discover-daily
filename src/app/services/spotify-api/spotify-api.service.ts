...
export class SpotifyApiService {
  ...
  getUserProfile(): Observable<UserObjectPublic> | Observable<null> {
  ...
  getPlaylists(
    userId: string,
    queryParams: IQueryParams = this.defaultQ
  ): Observable<PagingObject<PlaylistObjectSimplified>> | Observable<null> {
    return this.http.get<PagingObject<PlaylistObjectSimplified>>(`${this.apiBase}v1/users/${userId}/playlists?${this.q(queryParams)}`, {
      headers: this.h(),
    });
  }

  getSavedTracks(queryParams: IQueryParams = this.defaultQ): Observable<PagingObject<SavedTrackObject>> | Observable<null> {
    return this.http.get<PagingObject<SavedTrackObject>>(`${this.apiBase}v1/me/tracks?${this.q(queryParams)}`, {
      headers: this.h(),
    });
  }

  getRecommendedTracks(queryParams: RecommendationsOptionsObject): Observable<RecommendationsObject> | Observable<null> {
    return this.http.get<RecommendationsObject>(`${this.apiBase}v1/recommendations?${this.q(queryParams)}`, {
      headers: this.h(),
    });
  }

  getFilterMask(queryParams: { ids: string[] }): Observable<Array<boolean>> | Observable<null> {
    return this.http.get<Array<boolean>>(`${this.apiBase}v1/me/tracks/contains?${this.q(queryParams)}`, {
      headers: this.h(),
    });
  }

  createPlaylist(body: ICreatePlaylist, user: UserObjectPublic): Observable<PlaylistObjectFull> | Observable<null> {
  ...
  postTracksToPlaylist(body: IAddTracksToPlaylist, playlistId: string): Observable<PlaylistSnapshotResponse> | Observable<null> {
    return this.http.post<PlaylistSnapshotResponse>(`${this.apiBase}v1/playlists/${playlistId}/tracks?`, body, {
      headers: this.h().append('Content-Type', 'application/json'),
    });
  }

  replaceTracksInPlaylist(body: { uris: string[] }, playlistId: string): Observable<any> | Observable<null> {
    return this.http.put(`${this.apiBase}v1/playlists/${playlistId}/tracks?`, body, {
      headers: this.h().append('Content-Type', 'application/json'),
    });
  }

  uploadPlaylistCover(body: string, playlistId: string): Observable<any> | Observable<null> {
    return this.http.put(`${this.apiBase}v1/playlists/${playlistId}/images`, body, {
      headers: this.h().append('Content-Type', 'images/jpeg'),
    });
  }

  getTopArtists(queryParams?: IGetUserTopArtist): Observable<PagingObject<ArtistObjectFull>> | Observable<null> {
    return this.http.get<PagingObject<ArtistObjectFull>>(`${this.apiBase}v1/me/top/artists?${this.q(queryParams)}`, {
      headers: this.h(),
    });
  }

  getArtistsInformation(queryParams: { ids: string[] }): Observable<MultipleArtistsResponse> | Observable<null> {
